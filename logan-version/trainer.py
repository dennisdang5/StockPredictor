...
class Trainer():
    ...
    def train_one_epoch(self, epoch):
        ...
        for X_batch, Y_batch, indices in self.trainLoader:
            # move data to device
        ...
        with torch.no_grad():
            for X_batch, Y_batch, indices in self.validationLoader:
                X_batch = X_batch.to(self.device, non_blocking=self.pin_memory)
                Y_batch = Y_batch.to(self.device, non_blocking=self.pin_memory)
            ...
            stop_condition = self.stopper.early_stop(val_loss, self.Model)    
    ...
    def evaluate(self):
        ...
        with torch.no_grad():
            for X_batch, Y_batch, indices in self.testLoader:
                X_batch = X_batch.to(self.device, non_blocking=self.pin_memory)
                Y_batch = Y_batch.to(self.device, non_blocking=self.pin_memory)
