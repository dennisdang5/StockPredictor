...
# Also add logan-version and trained_models for backward compatibility
logan_version_dir = os.path.join(os.path.dirname(parent_dir), "logan-version")
trained_model_dir = os.path.join(os.path.dirname(parent_dir), "trained_models")
...
# Import from new-organization structure
...
class ModelEvaluator:
    ...
    def __init__(self, 
        ...
        if not os.path.isabs(model_path) and not os.path.exists(model_path):
            ...
            logan_version_dir = os.path.join(project_root, "logan-version")
            trained_models_dir = os.path.join(project_root, "trained_models")
            new_org_dir = parent_dir  # new-organization directory
            
            # Try in new-organization directory first (current structure)
            new_org_model_path = os.path.join(new_org_dir, model_path)
            if os.path.exists(new_org_model_path):
                model_path = new_org_model_path
            else:
                # Try in trained_models directory
                ...
                if os.path.exists(trained_model_path):
                    ...
                    if os.path.exists(logan_model_path):
                        ...
                        if os.path.exists(logan_trained_path):
                            ...
                            print(f"[Evaluator]    - New organization: {os.path.abspath(new_org_dir)}")
    ...
    def _save_sp500_cache(self, sp500_returns: np.ndarray, min_date, max_date):
        """
        ...
        cache_path = os.path.join(util.DATA_DIR, f"sp500_{cache_id}.npz")
    ...
    def _load_sp500_cache(self, min_date, max_date) -> Optional[np.ndarray]:
        """
    ...
    def _get_sp500_returns(self) -> Optional[np.ndarray]:
        ...
        sp500_returns = self._load_sp500_cache(min_date, max_date)
        ...
        try:
            ...
            # Save to cache
            self._save_sp500_cache(sp500_returns, min_date, max_date)
    ...
    def _load_model(self):
        """Load the saved model using the model registry."""
    ...
    def evaluate_paper_aligned_metrics(self,
        ...
        # Check if we have enough stocks per day for the requested k
        # Need at least 2*k stocks per day to construct portfolios
        df_temp = pd.DataFrame({
            "date": pd.to_datetime(self.test_dates),
            "pred": raw_predictions
        })
        min_stocks_per_day = df_temp.groupby("date").size().min()
        max_k = min_stocks_per_day // 2
        
        if k > max_k:
            print(f"[Evaluator] WARNING: Requested k={k} requires at least {2*k} stocks per day, "
                  f"but minimum stocks per day is {min_stocks_per_day}. "
                  f"Adjusting k to {max_k} to allow trading.")
            k = max_k
        ...
        paper_metrics = {
            ...
            'statistical_tests': {
                ...
                'dm_significant': dm_test.get('p_value', 1.0) < 0.05,  # Significant if p < 0.05
                'pt_statistic': pt_test['pt_statistic'],
                'pt_p_value': pt_test['p_value'],
                'pt_significant': pt_test.get('p_value', 1.0) < 0.05,  # Significant if p < 0.05
