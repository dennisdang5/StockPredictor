...
def evaluate_saved_model(model_path: str = "savedmodel_classification.pth", 
    ...
    if not os.path.exists(model_path):
        # Try in new-organization directory first
        ...
        elif os.path.exists(os.path.join(trained_models_dir, model_path)):
            model_path = os.path.join(trained_models_dir, model_path)
        # Try in logan-version directory
        elif os.path.exists(os.path.join(logan_version_dir, model_path)):
            model_path = os.path.join(logan_version_dir, model_path)
        # Try in logan-version/trained_models directory
        elif os.path.exists(os.path.join(logan_version_dir, "trained_models", model_path)):
            model_path = os.path.join(logan_version_dir, "trained_models", model_path)
        else:
            print(f"❌ Model file '{model_path}' not found!")
            print(f"   Searched in:")
            print(f"   - Current directory: {os.path.abspath('.')}")
            print(f"   - New organization: {os.path.abspath(new_org_dir)}")
    ...
    try:
        ...
        evaluator = ModelEvaluator(
            ...
            nlp_method=nlp_method if nlp_method else "aggregated",
